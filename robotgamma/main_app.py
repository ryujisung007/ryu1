...
with st.sidebar.expander("📅 데이터 동기화", expanded=True):
...
if st.session_state.get("records"):
    st.subheader("📋 품목제조보고 데이터 현황")
    st.dataframe(pd.DataFrame(st.session_state.records), use_container_width=True, height=200)
    ...
    if st.session_state.selected_flavor:
        ...
        with c1:
            b_type = st.selectbox("가공 방식", ["농축액", "NFC 과즙", "퓨레"])
        
        # 당감미 특성 라이브러리 정의
        sweet_lib = {
            "천연 감미 (Natural)": ["정백당", "결정과당", "꿀", "메이플시럽"],
            "저칼로리/기능성 (Functional)": ["액상알룰로스", "에리스리톨", "자일리톨", "D-소비톨"],
            "고감미/제로 (Zero-Sugar)": ["스테비아", "효소처리스테비아", "수크랄로스", "나한과"],
            "바디감/물성개선 (Body-Up)": ["물엿", "말토덱스트린", "올리고당", "쌀조청"]
        }
        
        with c2:
            s_category = st.selectbox("🍬 당감미 특성 선택", list(sweet_lib.keys()))
        with c3:
            s_choice = st.multiselect("📋 주요 사용당 선택", sweet_lib[s_category], default=[sweet_lib[s_category][0]])

        # AI 호출 및 데이터 무결성 체크
        ckey = f"v7_{st.session_state.selected_flavor}_{b_type}_{s_category}_{hash(tuple(s_choice))}"
        if ckey not in st.session_state:
            with st.status(f"AI 시니어 연구원이 {s_category} 기반 1,000포인트 분석 중...") as s:
                res = ai.propose_formula(st.session_state.selected_flavor, s_category, s_choice, b_type)
            ...
            for i, item in enumerate(others):
                with scols[i % 2]:
                    ...
                    v = st.slider(f"**{item['원료명']}** ({item['min']}%~{item['max']}%)", 
                                  ...
                                  key=f"sld_v7_{ckey}_{item['원료명']}")
                    adj[item['원료명']] = v
                    sum_others += v
            
            # [무결성] 정제수 오토 밸런싱 (합계 100.00% 유지)
